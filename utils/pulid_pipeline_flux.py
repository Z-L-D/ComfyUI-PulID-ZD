...
def PulIDPipelineFLUX(work_model, pulid, eva_clip, face_analysis, image, weight, start_at, end_at, method=None, noise=0.0, fidelity=None, projection=None, attn_mask=None):
    ...
    eva_clip.to(device, dtype=weight_dtype)
    pulid_model = pulid.to(device, dtype=weight_dtype)
    ...
    if attn_mask is not None:
        ...
        attn_mask = attn_mask.to(device, dtype=weight_dtype)
    ...
    for i in range(image.shape[0]):
        ...
        for size in [(size, size) for size in range(640, 256, -64)]:
            ...
            if face:
                ...
                iface_embeds = torch.from_numpy(face.embedding).unsqueeze(0).to(device, dtype=weight_dtype)
                break
        else:
        ...
        face_features_image = torchTransforms.functional.normalize(
            face_features_image.to(device, dtype=weight_dtype), 
            eva_clip.image_mean, 
            eva_clip.image_std
        )
        ...
        id_cond_vit = id_cond_vit.to(device, dtype=weight_dtype)
        for idx in range(len(id_vit_hidden)):
            id_vit_hidden[idx] = id_vit_hidden[idx].to(device, dtype=weight_dtype)
        ...
        uncond.append(pulid_model.get_image_embeds(id_uncond, id_vit_hidden_uncond)) #########!!!!!!!!!!!!! -- uncond_id_embedding = self.pulid_encoder(id_uncond, id_vit_hidden_uncond)
...
#     # average embeddings
    cond = torch.cat(cond).to(device, dtype=weight_dtype)
    uncond = torch.cat(uncond).to(device, dtype=weight_dtype)
    if cond.shape[0] > 1:
